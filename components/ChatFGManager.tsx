...
export default function ChatFGManager() {
  ...
  return (
    <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
      ...
      {/* Baris Filter */}
      <div className="flex flex-wrap gap-4 mb-8 p-4 bg-slate-50 rounded-2xl border border-slate-200">
        <select
          value={filterBulan}
          onChange={(e) => setFilterBulan(Number(e.target.value))}
          className="p-2 rounded-lg border text-sm text-slate-700"
        >
        ...
        <select value={filterFG} onChange={(e) => setFilterFG(e.target.value)} className="p-2 rounded-lg border text-sm text-slate-700">
        ...
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="p-2 rounded-lg border text-sm text-slate-700">
      ...
      <div className="grid gap-4">
        {filteredJadwal.length > 0 ? filteredJadwal.map((item) => {
          ...
          return (
            <div key={item.id} className={`p-6 border rounded-2xl flex justify-between items-center transition ${isSent ? 'bg-green-50 border-green-200' : 'bg-[#F0F7FF]/30 border-[#ADE1FB]/30'}`}>
              <div className="grid grid-cols-4 gap-4 flex-1">
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400">Fotografer</p>
                  <p className="font-bold text-[#0F2573]">{item.nama_fg || '-'}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400">Klien</p>
                  <p className="font-medium text-slate-700">{item.nama_klien || '-'}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400">Jadwal</p>
                  <p className="font-medium text-slate-700">{formatDate(item.tanggal)} - {item.jam?.substring(0, 5)}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase font-bold text-slate-400">Detail</p>
                  <p className="font-medium text-slate-700">{detail.kampus || '-'} / {detail.paket || '-'}</p>
                </div>
              </div>

              <button
                onClick={() => handleCopy(item)}
                className={`ml-6 px-5 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 ${isSent ? 'bg-green-500 text-white' : 'bg-[#266CA9] text-white hover:bg-[#0F2573]'}`}
              >
